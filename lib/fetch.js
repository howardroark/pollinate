...
function fetchURL(input, type) {
    ...
    if(type == "pollen") {
       meta.format = getResponseFormat(response);
    }
...
function fetchPath(input, type) {
    ...
    if(type == "pollen") {
       var extname = path.extname(tmp).replace('.','')
       meta.format = (extname) ? extname : 'json'
    }
...
function getResponseFormat(response) {
    var format = 'json'
    var extensions = ["yml","yaml","json","hjson"]
    var extname = path.extname(response.path).replace('.','')

    for(x in extensions) {
        if(extname == extensions[x]) {
            format = extname
        }
    }
